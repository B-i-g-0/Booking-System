...
namespace IConsult
{
    public partial class LecturerSignup : Form
    {
        ...
        private bool validate(bool blnValid)
        {
            ...
            if (txtLectNo.Text.Length < 8)
            {
                blnValid = false;
                MessageBox.Show("Minimum length of the lecturer number should be 8 characters", "ERROR");
            }
            if (txtLName.Text == "")
            {
                blnValid = false;
            ...
            {
                blnValid = false;
                MessageBox.Show("Please select school.", "ERROR");
                
            }
            if (txtOffice.Text == "")
            {
                blnValid = false;
                MessageBox.Show("Please enter office information.", "ERROR");
        ...
        private void btnSignUp_Click(object sender, EventArgs e)
        {
            ...
            if (blnValid == true)
            {
                conn.Close();
                conn.Open();
                OleDbCommand cmd1 = new OleDbCommand("select * from Lecturer where LecturerNo ='"+ txtLectNo.Text +"'", conn);
                OleDbDataReader dr = cmd1.ExecuteReader();
                if (dr.Read()){
                    MessageBox.Show("Profile already exists", "ERROR");
                }
                else {
                    OleDbCommand cmd = new OleDbCommand("insert into Lecturer values (@1, @2, @3, @4, @5, @6, @7)", conn);
                    cmd.Parameters.AddWithValue("@1", txtLectNo.Text);
                    ...
                    cmd.Parameters.AddWithValue("@5", txtOffice.Text);
                    cmd.Parameters.AddWithValue("@6", comboBox2.Text);
